...
const ProductSidebar = ({ show, onClose, onProductAction }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setBrands(brandRes.data.data.filter((item) => item.status === 1));
        setUnits(unitRes.data.data.filter((item) => item.status === 1));
        setCategories(
          categoryRes.data.data.filter((item) => item.status === 1)
        );
        setTags(tagRes.data.data.filter((item) => item.status === 1));
        setVariationList(
          variationRes.data.data.filter((item) => item.status === 1)
        );

        const branchOptions = branchRes.data.data
      ...
      <div className={`sidebar-div ${show ? "show" : ""}`}>
        <div className="p-4 bg-dark h-100 text-white overflow-auto">
          <div className="d-flex justify-content-between align-items-center">
            <h5 className="sidebar-title">Add Product</h5>
